...
Следует из операции merge для двух массивов. Чтобы объединить их в один, приходится выделять память на третий, временный, в котором приходится хранить значения какого-либо массива, чтобы в исходном изменение значений не влияло на merge. Если бы мы выделяли память под каждый merge, то сложность по времени была бы O(log_2_n * (n/2 + n/4 +… +2)) = O(nlogn).но можно сделать оптимальнее: завести временный массив как глобальную переменную и просто очищать ее перед каждым merge, передавая его в параметр метода. Худший случай выделения памяти – если объединяются два массива равных размеров, тогда придётся выделить O(n/2) = O(n) памяти. 
